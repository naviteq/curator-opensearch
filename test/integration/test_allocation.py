...
class TestActionFileAllocation(CuratorTestCase):
    def test_include(self):
        ...
        self.assertEquals(
            value,
            self.client.indices.get_settings(index="my_index")["my_index"]["settings"][
                "index"
        ...
        if ver >= (7, 10, 0):
            self.assertEquals(
                EMPTY710ROUTING,
                self.client.indices.get_settings(index="not_my_index")["not_my_index"][
                    "settings"
                ]["index"]["routing"],
            )
        else:
    ...
    def test_extra_option(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.bad_option_proto_test.format("allocation")
        )
        self.create_index("my_index")
        self.create_index("not_my_index")
        test = clicktest.CliRunner()
...
class TestCLIAllocation(CuratorTestCase):
    def test_include(self):
        ...
        self.assertEqual(
            0, self.run_subprocess(args, logname="TestCLIAllocation.test_include")
        )
        self.assertEquals(
            value,
            self.client.indices.get_settings(index="my_index")["my_index"]["settings"][
