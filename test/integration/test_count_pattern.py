...
class TestCLICountPattern(CuratorTestCase):
    ...
    def test_count_indices_by_age_same_age(self):
        ...
        self.assertEquals(
            value,
            self.client.indices.get_settings(index="c-2017.10.03")["c-2017.10.03"][
                "settings"
            ]["index"]["routing"]["allocation"][at][key],
        )
        self.assertEquals(
            value,
            self.client.indices.get_settings(index="d-2017.10.03")["d-2017.10.03"][
                "settings"
            ]["index"]["routing"]["allocation"][at][key],
        )
        idxlist = [
        ...
        for idx in idxlist:
            if ver >= (7, 10, 0):
                self.assertEquals(
                    EMPTY710ROUTING,
                    self.client.indices.get_settings(index=idx)[idx]["settings"][
                        "index"
                    ]["routing"],
                )
            else:
