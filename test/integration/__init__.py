...
host, port = os.environ.get("TEST_ES_SERVER", "127.0.0.1:9200").split(":")
...
class CuratorTestCase(TestCase):
    ...
    def create_indices(self, count, unit=None, ilm_policy=None):
        ...
        if not unit == "months":
            ...
            for _ in range(count):
                self.create_index(
                    self.args["prefix"] + now.strftime(format),
            ...
            for _ in range(1, count):
                ...
                self.create_index(
                    self.args["prefix"] + datetime(d.year, d.month, 1).strftime(format),
                    wait_for_yellow=False,
                    ilm_policy=ilm_policy,
                )
        # pylint: disable=E1123
    ...
    def create_index(
        ...
        self.client.indices.create(
            index=name, body=request_body, wait_for_active_shards=wait_for_active_shards
        )
        if wait_for_yellow:
            self.wfy()
    ...
    def create_repository(self):
        ...
        self.client.snapshot.create_repository(
            repository=self.args["repository"], body=body
        )
