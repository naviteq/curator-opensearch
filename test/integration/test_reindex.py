...
class TestActionFileReindex(CuratorTestCase):
    ...
    def test_reindex_from_remote(self):
        ...
        # Build remote client
        try:
            rclient = curator.get_client(host=rhost, port=rport, skip_version_test=True)
            rclient.info()
        except:
            raise SkipTest("Unable to connect to host at {0}:{1}".format(rhost, rport))
    ...
    def test_reindex_migrate_from_remote_with_pre_suf_fixes(self):
        ...
        self.assertEqual(
            expected,
            self.client.count(index="{0}{1}{2}".format(mpfx, source1, msfx))["count"],
        )
        self.assertEqual(
            expected,
            self.client.count(index="{0}{1}{2}".format(mpfx, source1, msfx))["count"],
