...
class TestActionFileClose(CuratorTestCase):
    ...
    def test_close_closed(self):
        ...
        self.client.indices.close(index="my_index", ignore_unavailable=True)
        self.create_index("dummy")
        test = clicktest.CliRunner()
        _ = test.invoke(
            curator.cli,
    ...
    def test_close_skip_flush(self):
        ...
        test = clicktest.CliRunner()
        _ = test.invoke(
            curator.cli, ["--config", self.args["configfile"], self.args["actionfile"]]
        )
        try:
            self.assertEquals(
                "close",
    ...
    def test_close_ignore_sync_failures(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.close_ignore_sync.format("true")
        )
        self.create_index("dummy")
        # Disable shard allocation to make my_index go red
        disable_allocation = (
    ...
    def test_close_has_sync_failures(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.close_ignore_sync.format("false")
        )
        self.create_index("dummy")
        # Disable shard allocation to make my_index go red
        disable_allocation = (
...
class TestCLIClose(CuratorTestCase):
    def test_close_delete_aliases(self):
        ...
        self.assertEquals(
            "close",
            self.client.cluster.state(index=index, metric="metadata")["metadata"][
                "indices"
            ][index]["state"],
        )
        self.assertEquals(
            "close",
            self.client.cluster.state(index="my_other", metric="metadata")["metadata"][
                "indices"
            ]["my_other"]["state"],
        )
        # Now open the indices and verify that the alias is still gone.
    ...
    def test_close_skip_flush(self):
        ...
        self.create_index("dummy")
        self.assertEqual(
            0, self.run_subprocess(args, logname="TestCLIClose.test_close_skip_flush")
        )
        self.assertEquals(
            "close",
            self.client.cluster.state(index="my_index", metric="metadata")["metadata"][
                "indices"
            ]["my_index"]["state"],
        )
