...
class cli_action:
    ...
    def __init__(
        self, action, client_args, option_dict, filter_list, ignore_empty_list, **kwargs
    ):
        ...
        if action[:5] != "show_":  # Ignore CLASS_MAP for show_indices/show_snapshots
            try:
                ...
                self.logger.critical(
                    "Action must be one of {0}".format(list(CLASS_MAP.keys()))
                )
    ...
    def do_singleton_action(self, dry_run=False):
        ...
        try:
            if self.action == "alias":
                ...
                action_obj = self.action_class(
                    self.client, *self.action_args, **self.action_kwargs
                )
            else:
            ...
            self.logger.critical(
                "Failed to complete action: {0}.  {1}: {2}".format(
                    self.action, type(err), err
                )
            )
            sys.exit(1)
