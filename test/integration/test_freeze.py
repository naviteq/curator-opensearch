...
class TestActionFileFreeze(CuratorTestCase):
    def test_freeze_unfrozen(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.optionless_proto.format("freeze")
        )
        ...
        self.assertEqual(
            "false",
            cluster_state["metadata"]["indices"]["my_index"]["settings"]["index"].get(
                "frozen", "false"
            ),
        )
        ...
        self.assertEqual(
            "false",
            cluster_state["metadata"]["indices"]["dummy"]["settings"]["index"].get(
                "frozen", "false"
            ),
        )
    ...
    def test_freeze_frozen(self):
        ...
        self.client.xpack.indices.freeze(index="my_index")
        self.create_index("dummy")
        cluster_state = self.client.cluster.state(
            metric="metadata",
        )
    ...
    def test_extra_option(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.bad_option_proto_test.format("freeze")
        )
        self.create_index("my_index")
        self.create_index("dummy")
...
class TestCLIFreeze(CuratorTestCase):
    def test_freeze_unfrozen(self):
        ...
        self.assertEqual(
            "false",
            cluster_state["metadata"]["indices"]["my_other"]["settings"]["index"].get(
                "frozen", "false"
            ),
        )
        ...
        self.assertEqual(
            0, self.run_subprocess(args, logname="TestCLIFreeze.test_freeze_unfrozen")
        )
        cluster_state = self.client.cluster.state(
            metric="metadata",
        )
