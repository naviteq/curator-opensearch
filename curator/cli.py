...
def process_action(client, config, **kwargs):
    ...
    if action == "alias":
        ...
        if "remove" in config:
            logger.debug('Removing indices from alias "{0}"'.format(opts["name"]))
            removes.iterate_filters(config["remove"])
            action_obj.remove(removes, warn_if_no_indices=opts["warn_if_no_indices"])
        if "add" in config:
...
def run(config, action_file, dry_run=False):
    ...
    for idx in action_keys:
        ...
        continue_if_exception = actions[idx]["options"].pop("continue_if_exception")
        logger.debug("continue_if_exception = {0}".format(continue_if_exception))
        ...
        if not allow_ilm and action not in settings.snapshot_actions():
            if actions[idx]["action"] == "rollover":
                ...
                try:
                    ...
                    if (
                        "name"
                        in idx_settings[write_index]["settings"]["index"]["lifecycle"]
                    ):
                        logger.info(
                            "Alias {0} is associated with ILM policy.".format(alias)
                        )
                        logger.info(
                            "Skipping action {0} because allow_ilm_indices is false.".format(
                                idx
                            )
                        )
                        continue
