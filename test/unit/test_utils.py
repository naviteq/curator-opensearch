...
class TestCheckCSV(TestCase):
    ...
    def test_check_csv_wrong_value(self):
        v = 123
        self.assertRaises(TypeError, curator.check_csv, v)
...
class TestGetDateRegex(TestCase):
    def test_non_escaped(self):
        self.assertEqual(
            "\\d{4}\\-\\d{2}\\-\\d{2}t\\d{2}", curator.get_date_regex("%Y-%m-%dt%H")
        )
...
class TestChunkIndexList(TestCase):
    ...
    def test_small_list(self):
        self.assertEqual(
            1, len(curator.chunk_index_list(["short", "list", "of", "indices"]))
        )
...
class TestGetIndices(TestCase):
    def test_client_exception(self):
        ...
        self.assertRaises(curator.FailedExecution, curator.get_indices, client)
...
class TestCheckVersion(TestCase):
    ...
    def test_check_version_less_than(self):
        client = Mock()
        client.info.return_value = {"version": {"number": "0.9.9"}}
...
class TestGetClient(TestCase):
    ...
    def test_url_prefix_none(self):
        kwargs = {"url_prefix": None, "use_ssl": True, "ssl_no_validate": True}
        self.assertRaises(curator.ClientException, curator.get_client, **kwargs)
    ...
    def test_master_only_multiple_hosts(self):
        ...
        self.assertRaises(curator.ConfigurationError, curator.get_client, **kwargs)
...
class TestGetRepository(TestCase):
    ...
    def test_get_repository_transporterror_negative(self):
        client = Mock()
        client.snapshot.get_repository.side_effect = opensearchpy.TransportError(
            503, "foo", "bar"
        )
        self.assertRaises(
            curator.CuratorException,
            curator.get_repository,
            client,
    ...
    def test_get_repository_notfounderror_negative(self):
        client = Mock()
        client.snapshot.get_repository.side_effect = opensearchpy.NotFoundError(
            404, "foo", "bar"
        )
        self.assertRaises(
            curator.CuratorException,
...
class TestGetSnapshot(TestCase):
    ...
    def test_get_snapshot_notfounderror_negative(self):
        client = Mock()
        client.snapshot.get_repository.return_value = testvars.test_repo
        client.snapshot.get.side_effect = opensearchpy.NotFoundError(
    ...
    def test_specified_snapshot_in_progress(self):
        ...
        self.assertEqual(
            ...
            curator.snapshot_in_progress(
                client, repository=testvars.repo_name, snapshot="snapshot-2015.03.01"
            ),
        )
...
class TestCreateSnapshotBody(TestCase):
    ...
    def test_create_snapshot_body_positive(self):
        self.assertEqual(
            testvars.snap_body, curator.create_snapshot_body(testvars.named_indices)
        )


class TestCreateRepoBody(TestCase):
    def test_missing_repo_type(self):
        self.assertRaises(curator.MissingArgument, curator.create_repo_body)
...
class TestCreateRepository(TestCase):
    def test_missing_arg(self):
        client = Mock()
        self.assertRaises(curator.MissingArgument, curator.create_repository, client)
    ...
    def test_raises_exception(self):
        client = Mock()
        client.snapshot.get_repository.return_value = {"not_your_repo": {"foo": "bar"}}
        client.snapshot.create_repository.side_effect = opensearchpy.TransportError(
...
class TestRepositoryExists(TestCase):
    def test_missing_arg(self):
        client = Mock()
        self.assertRaises(curator.MissingArgument, curator.repository_exists, client)
...
class TestRepositoryFs(TestCase):
    def test_passing(self):
        ...
        self.assertIsNone(curator.test_repo_fs(client, repository=testvars.repo_name))
...
class TestSafeToSnap(TestCase):
    def test_missing_arg(self):
        client = Mock()
        self.assertRaises(curator.MissingArgument, curator.safe_to_snap, client)
    ...
    def test_in_progress_fail(self):
        ...
        self.assertFalse(
            curator.safe_to_snap(
                client, repository=testvars.repo_name, retry_interval=0, retry_count=1
            )
        )
...
class TestSnapshotRunning(TestCase):
    ...
    def test_raises_exception(self):
        ...
        self.assertRaises(curator.FailedExecution, curator.snapshot_running, client)
...
class TestHealthCheck(TestCase):
    def test_no_kwargs(self):
        client = Mock()
        self.assertRaises(curator.MissingArgument, curator.health_check, client)

    def test_key_value_match(self):
        client = Mock()
        client.cluster.health.return_value = testvars.cluster_health
        self.assertTrue(curator.health_check(client, status="green"))

    def test_key_value_no_match(self):
        client = Mock()
        client.cluster.health.return_value = testvars.cluster_health
        self.assertFalse(curator.health_check(client, status="red"))

    def test_key_not_found(self):
        client = Mock()
        client.cluster.health.return_value = testvars.cluster_health
        self.assertRaises(
            curator.ConfigurationError, curator.health_check, client, foo="bar"
        )
...
class TestSnapshotCheck(TestCase):
    def test_fail_to_get_snapshot(self):
        ...
        self.assertRaises(curator.CuratorException, curator.snapshot_check, client)
    ...
    def test_in_progress(self):
        ...
        self.assertFalse(
            curator.snapshot_check(
                client, repository="foo", snapshot=testvars.snap_name
            )
        )
...
class TestRestoreCheck(TestCase):
    def test_fail_to_get_recovery(self):
        ...
        self.assertRaises(curator.CuratorException, curator.restore_check, client, [])

    def test_incomplete_recovery(self):
        client = Mock()
        client.indices.recovery.return_value = testvars.unrecovered_output
        self.assertFalse(curator.restore_check(client, testvars.named_indices))

    def test_completed_recovery(self):
        client = Mock()
        client.indices.recovery.return_value = testvars.recovery_output
        self.assertTrue(curator.restore_check(client, testvars.named_indices))
    ...
    def test_fix_966(self):
        ...
        self.assertTrue(curator.restore_check(client, testvars.index_list_966))
...
class TestTaskCheck(TestCase):
    def test_bad_task_id(self):
        ...
        self.assertRaises(curator.CuratorException, curator.task_check, client, "foo")
...
class TestWaitForIt(TestCase):
    ...
    def test_reindex_action_no_task_id(self):
        client = Mock()
        self.assertRaises(
            curator.MissingArgument, curator.wait_for_it, client, "reindex"
        )
    ...
    def test_restore_action_no_indexlist(self):
        client = Mock()
        self.assertRaises(
            curator.MissingArgument, curator.wait_for_it, client, "restore"
        )
...
class TestDateRange(TestCase):
    def test_bad_unit(self):
        self.assertRaises(
            curator.ConfigurationError, curator.date_range, "invalid", 1, 1
        )

    def test_bad_range(self):
        self.assertRaises(
            curator.ConfigurationError, curator.date_range, "hours", 1, -1
        )
    ...
    def test_hours_single(self):
        ...
        self.assertEqual(
            (start, end), curator.date_range(unit, range_from, range_to, epoch=epoch)
        )
    ...
    def test_weeks_single_iso(self):
        ...
        self.assertEqual(
            (start, end),
            curator.date_range(
                unit, range_from, range_to, epoch=epoch, week_starts_on="monday"
            ),
        )
...
class TestNodeRoles(TestCase):
    def test_node_roles(self):
        ...
        client.nodes.info.return_value = {
            "nodes": {node_id: {"roles": testvars.data_only_node_role}}
        }
...
class TestGetDateMath(TestCase):
    def test_success(self):
        ...
        client.indices.get.side_effect = opensearchpy.NotFoundError(
            404, "simulated error", {"error": {"index": expected}}
        )
        self.assertEqual(
            "hasthemath", curator.get_datemath(client, datemath, psuedo_random)
        )
    ...
    def test_failure(self):
        ...
        datemath = "{hasthemath}"
        client.indices.get.side_effect = opensearchpy.NotFoundError(
            404, "simulated error", {"error": {"index": "failure"}}
        )
        self.assertRaises(
            curator.ConfigurationError, curator.get_datemath, client, datemath
        )
...
class Test_process_auth_args(TestCase):
    ...
    def test_has_user_and_pass(self):
        ...
        self.assertEqual(
            ("user", "pass"), curator.utils.process_auth_args(test_data)["http_auth"]
        )
    ...
    def test_has_user_and_not_pass(self):
        ...
        test_data["username"] = "user"
        self.assertRaises(
            curator.ClientException, curator.utils.process_auth_args, test_data
        )
...
class Test_process_apikey_auth_args(TestCase):
    ...
    def test_has_invalid_apikey_auth(self):
        ...
        self.assertRaises(
            curator.ConfigurationError,
            curator.utils.process_apikey_auth_args,
            test_data,
        )
...
class Test_process_aws_args(TestCase):
    ...
    def test_null_aws_region(self):
        ...
        self.assertRaises(
            curator.MissingArgument, curator.utils.process_aws_args, test_data
        )
...
class Test_try_boto_session(TestCase):
    ...
    def test_get_credentials_fail(self):
        ...
        test_data["aws_sign_request"] = True
        self.assertRaises(
            botoex.NoCredentialsError, curator.utils.try_boto_session, test_data
        )
...
class Test_verify_master_status(TestCase):
    ...
    def test_master_only_raises_exception(self):
        ...
        self.assertRaises(
            curator.exceptions.ClientException,
            curator.utils.verify_master_status,
            client,
            master_only,
        )
