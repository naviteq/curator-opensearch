...
class TestActionFileShrink(CuratorTestCase):
    ...
    def test_shrink_non_green(self):
        ...
        self.assertEqual(
            2, len(indices)
        )  # Should have two indices, and a failed exit code (non-green)
        self.assertEqual(self.result.exit_code, 1)
    ...
    def test_shrink_with_extras(self):
        ...
        self.assertTrue(
            settings[self.idx]["settings"]["index"]["routing"]["allocation"]["require"][
                "_name"
            ]
            == ""
        )
        self.assertEqual(
            settings[self.target]["settings"]["index"]["codec"], "best_compression"
        )
        self.assertTrue(
            self.client.indices.exists_alias(index=self.target, name="my_alias")
        )
    ...
    def test_shrink_with_copy_alias(self):
        ...
        self.assertEqual(indices[0], self.target)
        self.assertTrue(
            self.client.indices.exists_alias(index=self.target, name=self.alias)
        )
...
class TestCLIShrink(CuratorTestCase):
    ...
    def test_shrink(self):
        ...
        self.assertEqual(
            0, self.run_subprocess(args, logname="TestCLIShrink.test_shrink")
        )
        indices = curator.get_indices(self.client)
        self.assertEqual(1, len(indices))  # Should only have `my_index-shrunken`
        self.assertEqual(indices[0], self.target)
