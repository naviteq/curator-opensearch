...
class TestActionFileRollover(CuratorTestCase):
    ...
    def test_extra_option(self):
        ...
        self.write_config(
            self.args["actionfile"], testvars.bad_option_proto_test.format("rollover")
        )
        test = clicktest.CliRunner()
        _ = test.invoke(
            curator.cli,
    ...
    def test_max_age_with_new_name_with_date(self):
        ...
        condition = "max_age"
        value = "1s"
        expected = {curator.parse_date_pattern(newindex): {"aliases": {alias: {}}}}
        self.client.indices.create(index=oldindex, body={"aliases": {alias: {}}})
        time.sleep(1)
        self.write_config(
            self.args["configfile"], testvars.client_config.format(host, port)
    ...
    def test_max_age_old_index_with_date_with_new_index(self):
        ...
        value = "1s"
        expected = {
            "%s" % curator.parse_date_pattern(newindex): {"aliases": {alias: {}}}
        }
        self.client.indices.create(index=oldindex, body={"aliases": {alias: {}}})
        time.sleep(1)
        self.write_config(
    ...
    def test_no_rollover_ilm_associated(self):
        ...
        if version <= (6, 6, 0):
            ...
            self.assertEqual(expected, len(self.client.indices.get_alias(name=alias)))
            self.assertEqual(
                oldindex, list(self.client.indices.get_alias(name=alias).keys())[0]
            )
...
class TestCLIRollover(CuratorTestCase):
    def test_max_age_true(self):
        ...
        self.assertEqual(
            0, self.run_subprocess(args, logname="TestCLIRollover.test_max_age_true")
        )
        self.assertEqual(expected, self.client.indices.get_alias(name=alias))
