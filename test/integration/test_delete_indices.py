...
class TestActionFileDeleteIndices(CuratorTestCase):
    ...
    def test_retention_from_name_days_ignore_failed_match(self):
        ...
        self.assertEquals(
            15, len(exclude_ilm_history(curator.get_indices(self.client)))
        )
    ...
    def test_retention_from_name_days_keep_exclude_false_after_failed_match(self):
        ...
        self.assertEquals(
            25, len(exclude_ilm_history(curator.get_indices(self.client)))
        )
    ...
    def test_retention_from_name_days_failed_match_with_fallback(self):
        ...
        self.assertEquals(
            12, len(exclude_ilm_history(curator.get_indices(self.client)))
        )
    ...
    def test_allow_ilm_indices_true(self):
        ...
        if curator.get_version(self.client) < (6, 6, 0):
            ...
            self.assertEquals(
                5, len(exclude_ilm_history(curator.get_indices(self.client)))
            )
...
class TestCLIDeleteIndices(CuratorTestCase):
    def test_name_older_than_now_cli(self):
        ...
        args += [
            ...
            "delete-indices",
            "--filter_list",
            '{"filtertype":"age","source":"name","direction":"older","timestring":"%Y.%m.%d","unit":"days","unit_count":5}',
        ]
        self.assertEqual(
            0,
            self.run_subprocess(
