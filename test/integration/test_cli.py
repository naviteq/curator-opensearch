...
class TestCLIMethods(CuratorTestCase):
    def test_bad_client_config(self):
        ...
        self.write_config(
            self.args["configfile"], testvars.bad_client_config.format(host, port)
        )
        self.write_config(
        ...
        result = test.invoke(
            curator.cli,
            ["--config", self.args["configfile"], "--dry-run", self.args["actionfile"]],
        )
    ...
    def test_no_config(self):
        ...
        localtest = False
        if (host == "localhost" or host == "127.0.0.1") and port == 9200:
            localtest = True
        self.create_indices(10)
    ...
    def test_no_logging_config(self):
        self.create_indices(10)
        self.write_config(
            self.args["configfile"], testvars.no_logging_config.format(host, port)
        )
        self.write_config(
            self.args["actionfile"],
            testvars.disabled_proto.format("close", "delete_indices"),
    ...
    def test_action_is_None(self):
        ...
        self.assertEqual(type(curator.ConfigurationError()), type(result.exception))
