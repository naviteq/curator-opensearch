...
if platform == "win32":
    ...
    print(
        "Your platform ({0}) is not yet supported for binary build/distribution.".format(
            platform
        )
    )
    sys.exit(1)
...
if os.path.exists(build_path):
    ...
    if os.path.exists(target_path):
        print(
            "An older build exists at {0}.  Please delete this before continuing.".format(
                target_path
            )
        )
        sys.exit(1)
    ...
    if os.path.exists(target_path):
        ...
        if float(pyver) >= 2.7:
            shutil.make_archive(
                "elasticsearch-" + target_name, archive_format, ".", target_path
            )
            if platform == "win32":
            ...
            print(
                "Your python version ({0}) is too old to use with shutil.make_archive.".format(
                    pyver
                )
            )
            print(
                "You can manually compress the {0} directory to achieve the same result.".format(
                    target_name
                )
            )
else:
    ...
    print(
        "Build not found.  Please run 'python setup.py build_exe' to create the build directory."
    )
    sys.exit(1)
